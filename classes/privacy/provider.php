...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        $collection->add_database_table('skillsoft_au_track', [
            ...
            ], 'privacy:metadata:skillsoft_au_track');

        $collection->add_database_table('skillsoft_session_track', [
          'userid' => 'privacy:metadata:userid',
          'sessionid' => 'privacy:metadata:session_track:sessionid',
          'timecreated' => 'privacy:metadata:session_track:timecreated'
        ], 'privacy:metadata:skillsoft_session_track');

        $collection->add_database_table('skillsoft_report_results', [
          ...
          'attempt' => 'privacy:metadata:attempt'
        ], 'privacy:metadata:skillsoft_report_results');

        $collection->add_database_table('skillsoft_tdr', [
        ...
        ], 'privacy:metadata:skillsoft_tdr');
    ...
    public static function get_contexts_for_userid(int $userid) : contextlist {
        ...
        $contextlist->add_from_sql(sprintf($sql1, 'skillsoft_au_track'), $params);
        $contextlist->add_from_sql(sprintf($sql1, 'skillsoft_session_track'), $params);
        $contextlist->add_from_sql(sprintf($sql2, 'skillsoft_tdr'), $params);
        $contextlist->add_from_sql(sprintf($sql2, 'skillsoft_report_results'), $params);
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        $userlist->add_from_sql('userid', sprintf($sqla, 'skillsoft_au_track'), $params);
        $userlist->add_from_sql('userid', sprintf($sqla, 'skillsoft_session_track'), $params);
        $userlist->add_from_sql('userid', sprintf($sqlb, 'skillsoft_tdr'), $params);
        $userlist->add_from_sql('userid', sprintf($sqlb, 'skillsoft_report_results'), $params);
    }
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        // The scoes_track data is organised in: {Course name}/{SkillSoft activity name}/{My attempts}/{Attempt X}/data.json
        ...
        array_walk($alldata, function($attemptsdata, $contextid) {
            ...
            array_walk($attemptsdata, function($data, $attempt) use ($context) {
                $subcontext = [
                    get_string('skillsoft_myattempts', 'skillsoft'),
                    get_string('skillsoft_attempt', 'skillsoft'). " $attempt"
                ];
        ...
        $sql = "SELECT ss.id,
                       ...
                       ss.timecreated,
                       ctx.id as contextid
                  FROM {skillsoft_session_track} ss
                  JOIN {course_modules} cm
                    ON cm.instance = ss.skillsoftid
                  JOIN {context} ctx
                    ON ctx.instanceid = cm.id
        ...
        array_walk($alldata, function($data, $contextid) {
            $context = \context::instance_by_id($contextid);
            $subcontext = [
                get_string('skillsoft_mysessions', 'skillsoft')
            ];
            writer::with_context($context)->export_data(
                $subcontext,
        ...
        array_walk($alldata, function ($data, $contextid) {
          $context = \context::instance_by_id($contextid);
          $subcontext = [
            get_string('skillsoft_mytdr', 'skillsoft')
          ];
          writer::with_context($context)->export_data(
            $subcontext,
            (object) ['TDRs' => $data]
        ...
        $sql = "SELECT ss.id,
                       ...
                       ss.attempt,
                       ctx.id as contextid
                   FROM {skillsoft_report_results} ss
                   JOIN {skillsoft} s
                     ON s.assetid = ss.assetid
                   JOIN {course_modules} cm
                     ON cm.instance = s.id
        ...
        // The scoes_track data is organised in: {Course name}/{SkillSoft activity name}/{My Report Results}/{Attempt X}/data.json
        // where X is the attempt number.
        array_walk($alldata, function ($attemptsdata, $contextid) {
          $context = \context::instance_by_id($contextid);
          array_walk($attemptsdata, function ($data, $attempt) use ($context) {
        $subcontext = [
            get_string('skillsoft_myreportresults', 'skillsoft'),
            get_string('skillsoft_attempt', 'skillsoft') . " $attempt"
            ];
            writer::with_context($context)->export_data(
              $subcontext,
