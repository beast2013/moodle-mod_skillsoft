...
class aicchandler {
	...
	public function exitau($return=false) {
		...
		if ($CFG->skillsoft_trackingmode == TRACK_TO_LMS) {
		...
		} elseif($CFG->skillsoft_trackingmode == TRACK_TO_OLSA_AUTOMATICSYNC) {

			//Request results from SkillSoft
			$resp = UD_GetAssetResults($this->user->username, $this->skillsoft->assetid);
			...
			if(($resp->success) && ($resp->result->RESULT->timesaccessed > 0)) {
				...
				$id = skillsoft_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]score', '');
				$id = skillsoft_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE_LESSON]', '');
				...
				$id = skillsoft_setFirstScore($this->user->id, $this->skillsoft->id, $this->attempt, $resp->result->RESULT->overallcurrentscore);
				$id = skillsoft_setCurrentScore($this->user->id, $this->skillsoft->id, $this->attempt, $resp->result->RESULT->overallcurrentscore);
				$id = skillsoft_setBestScore($this->user->id, $this->skillsoft->id, $this->attempt, $resp->result->RESULT->overallhighscore);
			}
		}
		$response = '';
		$response .= 'error=0'."\r\n";
